...
class McpOrchestrator:
    ...
    async def get_server_tools(
        ...
        try:
            ...
            # Create session key with project context
            session_key = f"{project_uuid}.{server_id}" if project_uuid else server_id
            logger.debug(f"Getting tools with session key: {session_key}")
            
            return await session_manager.get_server_tools(session_key, server_config, project_id=project_uuid)
    ...
    async def call_tool(
        ...
        try:
            ...
            # Create session key with project context
            session_key = f"{project_uuid}.{server_id}" if project_uuid else server_id
            logger.debug(f"Calling tool with session key: {session_key}")
            
            return await session_manager.call_tool(
                server_id=session_key,
